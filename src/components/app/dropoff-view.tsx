          ...
          <CardContent className="space-y-4">
            {dropOffLocations.map(location => (
              <div key={location.id} className="p-3 bg-muted/50 rounded-lg">
                <h3 className="font-semibold flex items-center gap-2"><MapPin className="w-4 h-4 text-primary" /> {location.name}</h3>
                <p className="text-sm text-muted-foreground mt-1 flex items-center gap-2"><Clock className="w-4 h-4"/> {location.hours}</p>
                <Button 
                  size="sm" 

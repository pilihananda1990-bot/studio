...
import { pets as allPets, Pet } from '@/lib/data/pets';
      ...
      <main className="flex-1 container mx-auto px-4 mt-4">
        <section className="relative rounded-2xl bg-gradient-to-r from-gray-800 to-gray-900 text-white p-6 overflow-hidden">
            <div className="relative z-10">
                <h1 className="text-2xl font-bold">40% Off on Pet Products</h1>
            ...
            <div className="grid grid-cols-2 gap-4">
                {filteredPets.map((pet, index) => (
                    <Link href="#" key={pet.id} className="block">
                        <div className={cn("rounded-2xl overflow-hidden", index % 2 === 0 ? "mt-0" : "mt-4")}>
                            <div className="relative h-40 w-full" style={{ backgroundColor: pet.backgroundColor }}>
                                <Image src={pet.image} alt={pet.name} layout="fill" objectFit="cover" data-ai-hint={`${pet.category} portrait`} />
                            </div>
                            <div className="p-3" style={{ backgroundColor: pet.backgroundColor }}>
                                <h4 className="font-bold text-lg">{pet.name}</h4>
                                <div className="flex gap-2 mt-2">
                                    <div className="text-xs font-semibold bg-white/50 rounded-full px-3 py-1">{pet.sex}</div>
                                    <div className="text-xs font-semibold bg-white/50 rounded-full px-3 py-1">{pet.age}</div>
                                </div>
                            </div>
                        </div>
                    </Link>
